...
import { parseBlock, planSwitch, profileFor, splitSession, type ModelsBlock } from "./roles.ts";
...
export default function (pi: ExtensionAPI) {
	...
	pi.registerCommand("provider", {
		...
		handler: async (args, ctx) => {
			...
			// registry's call — splitSession commits the split only when the bare
			// reference actually resolves, so a double miss persists the
			// configured string untouched, level and all.
			...
			const session = splitSession(plan.session, (reference) => find(reference) !== undefined);
			const target = session.resolved && session.reference ? find(session.reference) : undefined;

			const written = writeActive(agentDir, text, session.reference, session.thinking);
			if (!written.ok) {
			...
			if (plan.session) {
				if (!target) {
					report.push("", `The ${SESSION_ROLE} role names ${session.reference}, which is not a model pi knows. Session model unchanged.`);
					...
					report.push("", `Switched on disk, but ${session.reference} has no API key configured — the session model is unchanged.`);
				} else {
					...
					let note = "";
					if (session.thinking) {
						pi.setThinkingLevel(session.thinking as Parameters<typeof pi.setThinkingLevel>[0]);
						const applied = pi.getThinkingLevel();
						note = applied === session.thinking ? ` (thinking: ${session.thinking})` : ` (thinking: ${session.thinking}, applied as ${applied})`;
					}
					report.push("", `Session model is now ${session.reference}${note}.`);
				}
			} else {
