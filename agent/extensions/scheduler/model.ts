/**
 * Resolving the configured scheduler model reference to an actual model.
 *
 * A COPY of recap/model.ts, with this extension's setting name in the errors:
 ...
 * applies, so `scheduler.model` behaves like `--model` does:
 *
 ...
 *
 * Ambiguity is an error rather than a silent pick, because "the request was read
 * by a model I didn't expect" is worse than "the reference was ambiguous".
 *
 ...
 * ships `deepseek/deepseek-chat:free`). The level itself is ignored: the parse
 * call's thinking is pinned in parse.ts, so the suffix exists here only to be
...
function matchReference<M extends ModelLike>(reference: string, models: readonly M[]): Resolution<M> | undefined {
	...
	if (exact === "ambiguous") {
		return { ok: false, error: `scheduler.model "${reference}" matches more than one model — qualify it as provider/id` };
	}
	if (exact) return { ok: true, model: exact };

	const partial = partialMatch(reference, models);
	if (partial === "ambiguous") {
		return { ok: false, error: `scheduler.model "${reference}" matches several models — use a more specific id` };
	}
...
export function resolveModel<M extends ModelLike>(reference: string, models: readonly M[]): Resolution<M> {
	...
	if (custom === "ambiguous") return { ok: false, error: `scheduler.model "${reference}" matches several models of that provider — use a more specific id` };
	if (custom) return { ok: true, model: custom };

	// Every miss names the reference as configured — the suffix may be the typo.
	return { ok: false, error: `scheduler.model "${reference}" matched no available model` };
}

/**
 * Pick the model the /schedule parse call runs on.
 *
 * An explicit `scheduler.model` must resolve or the parse fails: the user named
 * it, and a silent stand-in would send their request somewhere they did not
