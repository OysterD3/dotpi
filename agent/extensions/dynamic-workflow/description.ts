...
Workflows run in the BACKGROUND: this call validates the script, starts the fleet, and returns immediately with a run id. A "workflow-result" message arrives when the run completes — NEVER fabricate or predict a pending run's results; continue with other work or end the turn and wait. The user watches progress in the status panel and can inspect, pause, resume or cancel runs from /workflows. Needing the result for your NEXT phase is not a reason to block: start the run, say what is in flight, and END YOUR TURN — the result message resumes the plan, and the user keeps the prompt for other work meanwhile. Reserve wait: true for two cases: the user explicitly asked to block, or the session cannot outlive your turn (print/json mode, a one-shot or scheduled invocation) — there, ending the turn ends the process and the result message would never land, so background work silently vanishes.
