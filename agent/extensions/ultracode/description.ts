...
**Nothing is capped.** There is no concurrency limit, no agent limit per run, no item limit on parallel()/pipeline(), and no wall-clock ceiling on an agent — an agent runs until it finishes or the user aborts. Every agent a script starts is spawned immediately, so a parallel() over fifty items launches fifty processes at once. That makes breadth free to ask for and yours to get right: fan out along real seams, and do not fan out to a number you would not want running simultaneously. Nested workflow() throws. The script body runs on the host event loop: always await — a synchronous busy-wait loop freezes the whole session.
